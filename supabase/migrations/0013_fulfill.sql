-- Phase 1 — Fulfill module (Sales pipeline step 4). ADDITIVE: one function, no DDL.
-- This is the FIRST stock cut: fulfill_order stamps order_lines.fulfilled_at, which moves
-- qty from stock_check.available into reserved (physical is unchanged — the box is still on
-- the shelf until Outbound ships it). shipped_at stays NULL here.
--
-- D1 is timestamp-driven: orders.status is NOT changed and no CHECK is altered. The picking
-- queue and the future Outbound worklist derive from fulfilled_at / shipped_at, not a status.
--
-- SECURITY INVOKER so the caller's RLS (is_allowed_user()) gates every write — the app calls
-- this with the anon key + the user's session, never the service-role key. service_role
-- (backend / smoke harness) may also call it.
