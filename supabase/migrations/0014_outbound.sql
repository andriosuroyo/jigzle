-- Phase 1 — Outbound (Ship) module (Sales pipeline step 5). The SECOND stock cut.
-- record_shipment stamps order_lines.shipped_at → physical (inbound−shipped) and reserved
-- (fulfilled−shipped) both drop; available is unchanged (it moved at fulfill). When every
-- non-cancelled line of an order is shipped, the order flips to 'Complete' (already in the
-- status enum — no CHECK change). shipped_at is the only stock change here; fulfilled_at and
-- holds are NOT touched.
--
-- a. Additive link/group columns, all NULLABLE so the 5,712 legacy outbound_shipments rows
--    (and any legacy boxes) are untouched and never backfilled.
-- b. record_shipment RPC — SECURITY INVOKER, pinned search_path (same posture as create_order
--    / fulfill_order): the app calls it with the anon key + the user's session, never service-role.
